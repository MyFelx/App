...
class API {
  static signUp(username, email, password, onSuccess, onFail) {
    return axios
      .post("http://3.141.165.79:5000/myFlex/api/v1/signgup", {
        username: username,
        email,
        password,
      })
  ...
  static isLoggedIn() {
    ...
    return axios
      .get("http://3.141.165.79:5000/myFlex/api/v1/user", {
  ...
  static login(loginValue, password, onSuccess, onFail) {
    return axios
      .post("http://3.141.165.79:5000/myFlex/api/v1/login", {
        loginValue,
        password,
      })
      .then((res) => {
  ...
  static loginAsGuest(onSuccess, onFail) {
    return axios
      .post("http://3.141.165.79:5000/myFlex/api/v1/login/guest")
      .then((res) => {
        localStorage.setItem("token", res.data.token);
        localStorage.setItem("user", JSON.stringify(res.data.user));
  ...
  static movieDetails(movieID) {
    return (
      axios
        .get(`http://3.141.165.79:5000/myFlex/api/v1/movie?searchQuery=${movieID}`)
        // .then((res) => console.log(res.data))
        .catch((e) => console.log(e))
    );
  }
  ...
  static addMovieToMyList(id) {
    return axios.patch(
      "http://3.141.165.79:5000/myFlex/api/v1/user/list",
      {
        id,
      },
      {
  ...
  static removeMovieFromMyList(id) {
    return axios.delete("http://3.141.165.79:5000/myFlex/api/v1/user/list", {
      headers: {
        Authorization: localStorage.getItem("token"),
      },
      data: {
  ...
  static getMyList() {
    return axios
      .get("http://3.141.165.79:5000/myFlex/api/v1/user/list", {
        headers: {
          Authorization: localStorage.getItem("token"),
        },
      })
